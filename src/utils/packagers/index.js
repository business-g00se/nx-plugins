...
exports.preparePackageJson = exports.packager = void 0;
...
function preparePackageJson(options, context, stats, resolverName, tsconfig) {
    ...
    devkit_1.logger.info('getting external modules');
    const workspacePackageJsonPath = path_1.join(context.root, 'package.json');
    ...
    devkit_1.logger.info('create a package.json with first level dependencies'); //First create a package.json with first level dependencies
    ...
    devkit_1.logger.info(`packager instance is -- ${options.packager}`);
    let dependencyGraph = null;
    // Get the packager for the current process.
    return rxjs_1.from(normalize_1.getProjectRoot(context)).pipe(operators_1.switchMap((root) => {
        options.root = path_1.join(context.root, root);
        ...
        if (packagerInstance === yarn_1.Yarn) {
            devkit_1.logger.info('generate lock entry for yarn for dependency graph to work.');
            const result = packagerInstance.generateLockFile(path_1.dirname(packageJsonPath));
            if (result.error) {
                devkit_1.logger.error('ERROR: generating lock file!');
        ...
        else if (packagerInstance === npm_1.NPM) {
            // need to install deps for dep-graph to work
            const result = packagerInstance.install(path_1.dirname(packageJsonPath));
            if (result.error) {
                devkit_1.logger.error('ERROR: generating lock file!');
                return rxjs_1.of({ success: false, error: result.error.toString() });
            }
        ...
        devkit_1.logger.info('get the packagelist with dependency graph and depth=2 level');
        ...
        if (getDependenciesResult.error) {
            devkit_1.logger.error('ERROR: getDependenciesResult!');
            return rxjs_1.of({
                success: false,
        ...
        if (options.verbose && !_.isEmpty(problems)) {
            devkit_1.logger.info(`Ignoring ${_.size(problems)} NPM errors:`);
            _.forEach(problems, (problem) => {
                devkit_1.logger.info(`=> ${problem}`);
            });
        }
        // re-writing package.json with dependency-graphs
        devkit_1.logger.info('re-writing package.json with dependency-graphs');
        ...
        devkit_1.logger.info('run packager to  install node_modules');
        ...
        if (packageInstallResult.error) {
            devkit_1.logger.error('ERROR: install package error!');
            return rxjs_1.of({
                success: false,
                error: packageInstallResult.error.toString(),
            });
        }
        devkit_1.logger.info(packageInstallResult.stdout.toString());

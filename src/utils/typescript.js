...
// projectDependencies: DependentLibraryNode[]
    ...
    return rxjs_1.Observable.create((subscriber) => {
        ...
        try {
            ...
            const tscPath = path_1.join(context.root, '/node_modules/typescript/bin/tsc');
            if (options.watch) {
                devkit_1.logger.info('Starting TypeScript watch');
                args.push('--watch');
                tscProcess = child_process_1.fork(tscPath, args, { stdio: [0, 1, 2, 'ipc'] });
                subscriber.next({ success: true });
            }
            else {
                devkit_1.logger.info(`Compiling TypeScript files for tsconfig ${tsConfigPath} under ${context.projectName} ${context.targetName}...`);
                tscProcess = child_process_1.fork(tscPath, args, { stdio: [0, 1, 2, 'ipc'] });
                tscProcess.on('exit', (code) => {
                    if (code === 0) {
                        devkit_1.logger.info(`Done compiling TypeScript files for tsconfig ${tsConfigPath} under ${context.projectName} ${context.targetName}`);
                        subscriber.next({ success: true });
                    }
                    else {

...
exports.consolidateExcludes = void 0;
...
function consolidateExcludes(options) {
    ...
    const parsedTSConfig = ts.readConfigFile(options.tsConfig, ts.sys.readFile).config;
    const appRoot = options.sourceRoot.replace('src', '');
    devkit_1.logger.info(`Adding excluding list to tsconfig ${excludeList}`);
    if (excludeList.length > 0) {
        /* Handle excludes for handlers */
        devkit_1.logger.info('Checking if exclude paths overlaps with handlers...');
        ...
        devkit_1.logger.warn(`you are left with --> ${JSON.stringify(options.files)}`);
    ...
    devkit_1.logger.info(`writing tsconfig.serverless.nx-tmp with added excludeLists to ${tmpTsConfigPath}`);
